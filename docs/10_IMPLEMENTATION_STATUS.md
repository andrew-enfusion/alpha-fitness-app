...
- Phase 2 onboarding and profile: in progress. The deterministic baseline slices are complete, and the dual-target guidance foundation is now in place: local `UserProfile` persistence flows through Room, repository, and use-case layers; the onboarding state and screen shell are in place; the app derives and persists a baseline calorie target locally using deterministic profile inputs; onboarding persists a separate `NutritionGuidance` record for the working target and explanation; and the current local guidance path now supports baseline-vs-working-target differentiation plus reset-to-baseline behavior.
...
- The next smallest valid task is the provider-backed AI onboarding slice: replace the local development guidance gateway with a real structured AI guidance path while preserving the explicit dual-target ownership and reset-to-baseline behavior.
