...
- Phase 3 text meal logging core: in progress. The first three Log-only slices are complete: Log now has a dedicated `ViewModel`, explicit UI events, message-input state, a submit action wired through a small use case, a structured interpretation/review-ready state driven through a `ViewModel -> UseCase -> Repository/Gateway` path, and an explicit confirm-save flow that persists `MealEntry` and `MealItem` records to Room before recomputing and replacing `DailyMetrics` for the affected date from scratch.
...
- The next smallest valid task is the next Log-only Phase 3 slice: replace the current stubbed local-first interpretation path with real local meal-memory matching sourced from confirmed saved meals, while still deferring FoodReference write-back and photo flow work.
