...
- Phase 3 text meal logging core: in progress. The first two Log-only slices are complete: Log now has a dedicated `ViewModel`, explicit UI events, message-input state, a submit action wired through a small use case, and a structured interpretation/review-ready state driven through a `ViewModel -> UseCase -> Repository/Gateway` path. Local-first lookup is executed first in the use case, currently returns empty by design, and then falls back to a minimal structured gateway interpretation without introducing confirmed meal persistence or transcript-style chat history yet.
...
- The next smallest valid task is the next Log-only Phase 3 slice: add confirmed meal save flow from the review-ready Log state and recompute metrics on save without broadening into photo, calendar, or insight work.
